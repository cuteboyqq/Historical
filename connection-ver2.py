...
class SOCKET:
    ...
    def receive_image_and_log(self, client_socket, custom_directory=None):
        try:
            ...
            image_path = self.save_image(buffer, frame_index)
    ...
    def receive_image_and_log_and_imgpath(self, client_socket):
        try:
            frame_index = self.receive_frame_index(client_socket)
            size = self.receive_image_size(client_socket)
            image_data = self.receive_image_data(client_socket, size)
            self.save_image(image_data, frame_index)
    ...
    def _process_json_data(self, json_data, addr):
        """
    ...
    def _handle_client_connection(self, client_socket, device_mode, visual_mode, custom_directory=None):
        """
        ...
        """
        logging.info(f"device_mode : {device_mode}")
        logging.info(f"visual_mode : {visual_mode}")
        if visual_mode == 'semi-online':
            json_data = self._receive_json_data(client_socket)
            self._process_json_data(json_data, None)
        elif visual_mode == 'online' and device_mode == 0: # Device live mode
            self.receive_image_and_log(client_socket,custom_directory=None)
...
class Connection(BaseDataset):
    ...
    def start_server(self, visual_mode='online', custom_directory=None):
        ...
        Args:
            ...
            custom_directory (str): Custom directory for saving data, if applicable.
        """
        # server_socket = self.SOCKET._create_server_socket()
        """
        Start an improved version of the TCP server to handle image and JSON log reception.
        """
        server_socket = socket.socket(socket.AF_INET, socket.SOCK_STREAM)
        ...
        try:
            # self.SOCKET._bind_server_socket(server_socket)
            # logging.info(f"🟢 Server started on {self.tftp_ip}:{self.server_port}")
            # logging.info("🔍 Server is running. Waiting for connections...")

            # device_mode = self.get_input_mode()
            # logging.info(f"🔍 Device input mode is {device_mode}")
            ...
            while not self.stop_server.is_set():  # Check if the stop signal is set
                try:
                    client_socket, addr = server_socket.accept()
                    logging.info(f"🔗 Connection established with {addr}")
                    self.SOCKET._handle_client_connection(client_socket, device_input_mode, visual_mode, custom_directory)
                    client_socket.close()
                    logging.info(f"🔒 Connection closed with {addr}")
